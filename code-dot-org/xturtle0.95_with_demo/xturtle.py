...
_ver = "xturtle0.95a0 - bugfixes: 1"
# bugfix nr 1, (reported by john Posner, 15. 3. 2008):
#           weird behavour of undo() with fill and redo fill
#               
print _ver
...
import types
import math, sys, time, os     ## for compatibility: from math import *   ???
...
def _readconfig(cfgdict = _CFG):
    default_cfg = "xturtle.cfg"
    if isfile(default_cfg):
        cfg_file = default_cfg
        print "Loading config-file from cwd:", os.getcwd()
    else:
        head, tail = split(__file__)
        cfg_file = join(head, default_cfg)
        if not isfile(cfg_file):
            print "No config file!"
            return
        print "Loading config-file from:", head
    f = open(cfg_file, "r")
...
class TurtleScreenBase(object):
    """Provides the basic graphics functionality.
       ...
       To port xturtle.py to some other graphics toolkit
...
#### Rudiments of Error handling with these classes
...
class Terminator (Exception):
    ...
    in the Demo-Viewer xturtle.Demo.py.
    """
    pass
...
class TurtleScreen(TurtleScreenBase):
    ...
    All those methods are also transferred to the RawTurtle
    class, so theys can be called as RawTurtle methods or
    ...
    def __init__(self, cv):   
        self._shapes = { 
                ...
                "triangle" : Shape("polygon", ((10,-5.77), (0,11.55),
                              (-10,-5.77))),
                  "classic": Shape("polygon", ((0,0),(-3,-8),(0,-6),(3,-8))),
    ...
    def _color(self, args):
        ...
        if isinstance(args, str):
            raise TG_Error, "bad color sequence: %s" % `args` 
        try:
            r, g, b = args
        except:
            raise TG_Error, "bad color arguments: %s" % `args`
...
class TNavigator(object):
    ...
    def _goto(self, end):
        ...
        if self._odometerOn:
            self._odometer += self.distance(end)
        self._position = end
...
class TPen(object):
    ...
    def resizemode(self, rmode=None):
        ...
        resizemode("user") is called by a call of turtlesize with arguments.
        ---
    ...
    def pensize(self, width=None):
        ...
        if self.undobuffer:
            self.undobuffer.push(("pens", self._pensize))
        self._newLine()
        self._pensize = width
        self._update()
    ...
    def pencolor(self, *args):
        ...
        if args:
            if self.undobuffer:
                self.undobuffer.push(("pcol", self.pencolor()))
            color = self._color(args)
    ...
    def fillcolor(self, *args):
        ...
        if args:
            if self.undobuffer:
                self.undobuffer.push(("fcol", self.fillcolor()))
            color = self._color(args)
    ...
    def pen(self, pen=None, **pendict):
        ...
        if "pencolor" in p:
            if isinstance(p["pencolor"], tuple):
                p["pencolor"] = self._color((p["pencolor"],))            
        ...
        if "fillcolor" in p:
            if isinstance(p["fillcolor"], tuple):
                p["fillcolor"] = self._color((p["fillcolor"],))            
    ...
    def turtlesize(self, stretchx=None, stretchy=None, outline=None):
        ...
        """
#        print "turtlesize-args", stretchx, stretchy, outline
        if stretchx is None and stretchy is None and outline == None:
            stretchx, stretchy = self._stretchfactor
            return stretchx, stretchy, self._outlinewidth
        if stretchx is not None:
            if stretchy is None:
                self._stretchfactor = stretchx, stretchx
            else: 
                self._stretchfactor = stretchx, stretchy
        elif stretchy is not None:
            stretchx = self._stretchfactor[0]
            self._stretchfactor = stretchx, stretchy
        if outline is not None:
    ...
    def tilt(self, angle):
        ...
        """
        if angle == None:
            return self._tilt * (180.0/math.pi) / self._degreesPerAU
        w = angle * self._degreesPerAU * math.pi / 180.0
        self._tilt = (self._tilt + w) % (2*math.pi)
        self._update()

    def settilt(self, angle=None):
        ...
        >>> turtle.fd(50)
        """
        self._tilt = (angle * self._degreesPerAU * math.pi / 180.0) % (2*math.pi)
        self._update()
...
class _TurtleImage(object):
    ...
    def _setshape(self, shapeIndex):
        screen = RawTurtle.screens[self.screenIndex]
...
class RawTurtle(TPen, TNavigator):
    ...
    def __init__(self, canvas, shape="arrow", mode="standard",
        ...
        self.stampItems = []
        self.undobuffer = Tbuffer(undobufsize)
        self._update()
    ...
    def clear(self):
        ...
        Does not move the pen. Drawings of other pens
        are not affected.
...
##
## Bug fix contributed by Brad Miller
##
##        if forced:
...
##                                 self._pencolor, self._pensize)
    ...
    def _cc(self, args):
        ...
        try:
            ...
            raise TG_Error, "bad color arguments: %s" % `args`
        if self.getscreen()._colormode == 1.0:
            r, g, b = [round(255.0*x) for x in (r, g, b)]
        if not ((0 <= r <= 255) and (0 <= g <= 255) and (0 <= b <= 255)):
            raise TG_Error, "bad color sequence: %s" % `args`        
        return "#%02x%02x%02x" % (r, g, b)
...
##############################  stamp stuff  ###############################
        ...
        if stampid in self.stampItems:
            if isinstance(stampid, tuple):
                for subitem in stampid:
                    screen._delete(subitem)
            else:
                screen._delete(stampid)
    ...
    def _undogoto(self, entry):
        ...
        odometer, drawing, pc, ps, filling = go_modes
        ...
        self.currentLine = cL
        screen._drawline(cLI, pl, fill=pc, width=ps)
    ...
    def getscreen(self):
        ...
        drawing  on. Some of the RawTurtle methods can also
    ...
    def onscreenclick(self, fun, btn=1, add=None):
        ...
        ### event-binding will be removed
        """
        self.getscreen().onclick(fun, btn, add)
    ...
    def onclick(self, fun, btn=1, add=None):
        ...
        ###event-binding will be removed
        """
        self.getscreen()._onclick(self.turtle._item, fun, btn, add)
        self._update()
    ...
    def onrelease(self, fun, btn=1, add=None):
        ...
        """
        self.getscreen()._onrelease(self.turtle._item, fun, btn, add)
        self._update()
    ...
    def ondrag(self, fun, btn=1, add=None):
        ...
        ### (if pen is down).
        """
        self.getscreen()._ondrag(self.turtle._item, fun, btn, add)
    ...
    def _undo(self, action, data):
        ...
        if action == "rot":
            ...
            screen = self.screens[self.screenIndex]
            item = data[0]
            screen._drawpoly(item, ((0,0),(0,0),(0,0)), fill="", outline="")
            ...
            TPen.pencolor(self,data[0])
            self.undobuffer.pop()
        elif action == "fcol":
            TPen.fillcolor(self,data[0])
            self.undobuffer.pop()
        elif action == "pens":
            TPen.pensize(self,data[0])
            self.undobuffer.pop()

    def undo(self):
        """undo (repeatedly) the last turtle action.
        Number of available undo actions is determined by size of
        the undobuffer.
    ...
    # consider the following three aliases as deprecated
    polystart = begin_poly   
    polyend = end_poly
    getpoly = get_poly
...
class Turtle(RawTurtle):
    ...
    def __init__(self, shape=_CFG["shape"],
                       mode=_CFG["mode"],
                       width=_CFG["width"],
                       height=_CFG["height"],
                       ...
                       visible=True):
        """Erzeugt Turtle auf einem Standard-ScrolledCanvas
        """
        ...
        if _canvas is None:
            _canvas = ScrolledCanvas(_root, width=width, height=height,
                                            canvwidth = canvwidth,
                                            canvheight = canvheight)
##            self.winsize(width, height, leftright, topbottom)
            self.setup(width, height, leftright, topbottom)
            _canvas.pack(expand=1, fill="both")
        RawTurtle.__init__(self, _canvas, shape=shape, mode=mode,
                           visible=visible, undobufsize=undobuffersize)

    def winsize(self, w, h, lr=_CFG["leftright"], tb=_CFG["topbottom"]):
    ...
    def _destroy(self):
        global _root, _canvas, _pen, screen
        screen = self.screens[self.screenIndex]        
...
## Provisorium - durch "Wiener Desktop Fassung" ersetzen
def title(titlestring):
    """Set title of xturtle-window
    """
    global _root, _title
    if _root is not None:
        _root.title(titlestring)
    _title = titlestring


###  stuff for providing translation of docstrings
###  for methods of RawTurtle

    
def write_docstrings(cls):
    """Helper-Method to create doc-dictionary,
    has to called explicitely, (not used by the turtle-graphics classes)
    """
    f = open(cls.__name__+"_docsdict.py","w")
    f.write('docsdict = {\n\n')
    for key in cls.__dict__:
        if key.startswith('_'):
            continue
        f.write('%s :\n' % repr(key))
        f.write('"""%s\n""",\n\n' % eval('%s.%s.__doc__'%(cls.__name__,key)))
    f.write("}\n")
    f.close()


def read_docstrings(lang):
    """Transfer docstrings, translated to lang,
    from a dictionary-file to the methods of the above classes.
    """
    modname = "docsdict_%(language)s" % {'language':lang.lower() }
    module = __import__(modname) 
    docsdict = module.docsdict
    for key in docsdict:
        for cls in [Turtle, RawTurtle, TNavigator, TPen]:
            if key not in cls.__dict__:
                continue
            eval('%s.%s' % (cls.__name__, key)).im_func.__doc__ = docsdict[key]
...
def _docrevision(docstr):
    ...
    parexp = re.compile(r' \(.+ turtle\):')
    newdocstr = parexp.sub(":", newdocstr)
##    newdocstr = newdocstr.replace(" (for a Turtle instance named turtle)","")
    return newdocstr
...
for _cls in TNavigator, TPen, RawTurtle, Turtle:
    for _key in  _cls.__dict__.keys():
