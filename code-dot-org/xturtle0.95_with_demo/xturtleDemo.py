...
def getExampleEntries():
    cwd = os.getcwd()
    if "xturtleDemo.py" not in os.listdir(cwd):
        print "Directory of xturtleDemo must be current working directory!"
        print "But in your case this is", cwd
        sys.exit()
    entries1 = [entry for entry in os.listdir(cwd) if
                     entry.startswith("xtx_") and
    ...
    for entry in entries1:
        if entry.endswith(".py"):
            ...
            scripts = [script for script in os.listdir(path) if
                            script.startswith("xtx_") and
                            script.endswith(".py")]
            entries2.append(subdir+scripts)
    return entries2

def showWhatsNew():
    TextViewer(demo.root, "What'S new in xturtle.py, version 0.95a", "whatsnew.txt")

def showDemoHelp():
    TextViewer(demo.root, "Help on xturtleDemo", "demohelp.txt")
    
def showXturtleHelp():
    TextViewer(demo.root, "Help on xturtle.Turtle", "xturtle-docs.txt")
    
def showAboutDemo():
    TextViewer(demo.root, "Help on xturtleDemo", "about_xtdemo.txt")

def showAboutXturtle():
    TextViewer(demo.root, "Help on xturtleDemo", "about_xturtle.txt")

class DemoWindow(object):

    def __init__(self, filename=None):   #, root=None):
        self.root = root = xturtle._root = Tk()
        ...
        self.scanvas = xturtle._canvas = xturtle.ScrolledCanvas(g_frame,
                                       width=800, height=600,
                                       canvwidth=1600, canvheight=1200)
    ...
    def makeLoadDemoMenu(self):
        ...
        for entry in getExampleEntries():
            ...
            if isinstance(entry,str):
                CmdBtn.menu.add_command(label=entry[4:-3], underline=0, font=menufont,
                                        command=loadexample(entry))
            else:
                _dir, entries = entry[0], entry[1:]
                CmdBtn.menu.choices = Menu(CmdBtn.menu)
                for e in entries:
                    CmdBtn.menu.choices.add_command(label=e[4:-3], underline=0, font=menufont,
                              command = loadexample(os.path.join(_dir,e)))

                CmdBtn.menu.add_cascade(label=_dir[4:],
    ...
    def makeHelpMenu(self):
        ...
        CmdBtn.menu.add_command(label='Whats new in 0.95', font=menufont, command=showWhatsNew)
        CmdBtn.menu.add_command(label='About xturtle', font=menufont, command=showAboutXturtle)
        CmdBtn.menu.add_command(label='xturtle - Help', font=menufont, command=showXturtleHelp)
        CmdBtn.menu.add_command(label='xturtleDemo - Help', font=menufont, command=showDemoHelp)
        CmdBtn.menu.add_command(label='About xturtleDemo', font=menufont, command=showAboutDemo)
    ...
    def refreshCanvas(self):
        ...
        xturtle._pen=None
        self.scanvas.pack_forget()
        self.btn_frame.pack_forget()
        self.graph_frame.pack_forget()
        self.scanvas.destroy()
        self.scanvas = xturtle._canvas = xturtle.ScrolledCanvas(self.graph_frame,
                                       width=800, height=600,
                                       canvwidth=1600, canvheight=1200)
        self.scanvas.pack(side=TOP, fill=BOTH, expand=1)
        self.btn_frame.pack(side=TOP, fill=BOTH, expand=0)
    ...
    def loadfile(self,filename):
        ...
        if os.path.exists(filename) and not os.path.isdir(filename):
            ...
            self.root.title(fname[4:-3]+" - an xturtle example")
    ...
    def startDemo(self):
        self.refreshCanvas()
        self.dirty = True
        xturtle.TurtleScreen._RUNNING = True
    ...
    def stopIt(self):
        if self.exitflag:
            ...
            xturtle.TurtleScreen._RUNNING = False
